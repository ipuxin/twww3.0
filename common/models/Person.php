...
class Person extends \yii\db\ActiveRecord
{
    ...
    {
        return [
//            [[  'cityname', 'realname', 'mobile', 'email', 'password'  ], 'required'],
            [['cityname', 'realname', 'mobile', 'email', 'password', 'personid', 'business',
                'personpica', 'personpicb', 'person_id_andheadpic', 'shopname', 'shoplogo', 'address'], 'required'],
            [['status', 'created_at', 'updated_at'], 'integer'],
            [['invitation', 'username', 'cityname', 'realname', 'mobile', 'shopinfo',
                'email', 'password', 'personid', 'business', 'personpica', 'personpicb', 'person_id_timestar',
                'person_id_timeend', 'person_id_andheadpic', 'person_halfbody', 'trademark', 'brand_certificate',
                'qc_report', 'qc_other', 'shopname', 'shoplogo', 'address', 'zhifubaoaccount', 'type'], 'string', 'max' => 255],
            [['username'], 'unique'],
            [['mobile'], 'unique'],
            [['email'], 'unique'],
            [['personid'], 'unique'],
...
//            $this->username = Yii::$app->user->identity->username;
            if ($insert) {
                ...
                //打印日志
                $this->logResultMy(json_encode('---insert---'));
                $this->logResultMy(json_encode($insert));
