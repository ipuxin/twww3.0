...
<?= $form->field($model, 'email')->textInput(['maxlength' => true, 'placeholder' => "请输入常用邮箱，便于审核信息及后期规则的及时送达"]) ?>

<?= $form->field($model, 'password')->passwordInput(['maxlength' => true, 'placeholder' => "请输入后台登录密码"]) ?>

<?= $form->field($model, 'personid')->textInput(['maxlength' => true, 'placeholder' => "请输入管理人的证件号（身份证、护照、军官证等均可）"]) ?>
